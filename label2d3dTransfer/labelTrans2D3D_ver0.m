...
params.frmSta = 70;   % starting frame
...
for cIdx = params.frmSta:procfrm:params.frmEnd
    cIdx
    if mod(cIdx, 10)==0
        save(sprintf('cIdx_%03d.mat', cIdx));
    end
    ...
    cloudRegist = Tmapping*cloudRegist;
    ...
    if cIdx == 26
        display('debug code here...');
    end
    %% Load detection results
    labelFile = sprintf('%s%06d_yolo.txt', paramCfg.imgDir,cIdx+shiftIdx);
    bbox = []; objLabels = []; objProb = [];
    [bbox(:,1), bbox(:,2), bbox(:,3), bbox(:,4), objProb, objLabels] = ...
        textread(labelFile,'%d %d %d %d %f %s');
    bbNb = size(bbox, 1);
    boxLabel = 0;
    bbCandidate = getUniqueBBox( cloudNoGndFoV, cloudNoGndProj3DUnique,...
        velo3dRegFoV, paramCfg.P_velo_to_img, Tmapping, tform,...
        bbox, objLabels, objProb, bbNb, cIdx);
    %% Update object List of 3d Map
    [objectList] = updateObjectList(objectList, bbCandidate);
    %% Update object speed
    [objectList] = updateObjectListMotionInfo(objectList, noGndRegistT0, ...
         noGndRegistT1, cloudRegist, tform, Tmapping, paramCfg.P_velo_to_img, leftImgRef);
    %% Show Map
...
%     pcshow(velo3dRegMap(1:3,:)', uint8(velo3dRegFoV_Color), 'MarkerSize', 15); drawnow;
    ...
    objectList = drawObjectList(objectList, colors, leftImgRef);
    figure(1001); hold on; drawnow;
        saveas(gcf, sprintf('%s%06d_mapLabels.png','./label3d/',cIdx+shiftIdx));
	hold off;
    continue;
    %%
    if 0
        for iplot = 1:length(bbCandidate)
            bbInfo = bbCandidate{iplot};
            bboxCam = bbInfo.bb3dCam;
            bboxMap = Tmapping*pinv(tform.T')*[bboxCam'; ones(1, 8)]; % bounding box in map frame
            bboxMap = bboxMap(1:3, :)';
            colorDraw = getDrawColor(colors, bbInfo.label);
            figure(1000), hold on; plotCube(bboxMap, colorDraw); hold off;
            figure(1001), hold on; rectangle('Position', bbInfo.bb2d,...
